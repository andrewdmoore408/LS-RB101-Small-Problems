...
def longest_sentence
  # text = YAML.load_file('Frankenstein.txt')
  text = File.open('Frankenstein.txt').read

  text = text.gsub("  ", " ")

  sentences = text.split(/[.!?]/).map! { |sentence| sentence.strip }

  sentence_words = []

  sentences.each { |sentence| sentence_words.push(sentence.split) }

  word_count = sentence_words.max_by { |sentence| sentence.length }.length

  puts "The longest sentence in this text is #{word_count} words long."
end
